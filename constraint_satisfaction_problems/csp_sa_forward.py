...
class CSP:
    ...
    def backtracking_search(self, use_forward_checking: bool = False):
        return self.recursive_backtracking({}, copy.deepcopy(self.domains), use_forward_checking)
    ...
    def forward_check(self, var, value, assignment, domains):
        ...
        for neighbor in self.neighbours[var]:
            ...
            if not revised_domain:
                return False  # Domain wipe-out: trigger backtrack
...
def main():
    ...
    result = csp.backtracking_search(use_forward_checking=True)
    if result:
