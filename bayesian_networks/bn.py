...
class BayesianNetwork(object):
    ...
    #         value_str = values[var_name]
    #         value_bool = value_str == "True"  # convert string to boolean

    #         parent_values = self.sub_vals(variable, values)
    #         val_str = "true" if value_bool else "false"

    #         prob = variable.get_probability(val_str, parent_values)
    #         joint *= prob
    ...
    def get_conditional_probability(self, values: dict[str, str], evidents: dict[str, str]) -> float:
        ...
        if all(first_variable.is_child_of(self.variable_dictionary[evident]) for evident in evidents.keys()):
            ...
            for child, c_val in values.items():
                res *= self.variable_dictionary[child].get_conditional_probability(
                    c_val, evidents)
        ...
        # assumption: nodes in each level are independent, given their parents
            ...
            for parent, p_val in values.items():
                joint_marginal_parents *= self.variable_dictionary[parent].get_marginal_probability(
                    p_val)
            ...
            for child, c_val in evidents.items():
                joint_marginal_children *= self.variable_dictionary[child].get_marginal_probability(
                    c_val)

                # children given their parents. here the values become the
                # evidents!
                joint_conditional_children *= self.variable_dictionary[child].get_conditional_probability(
                    c_val, values)
